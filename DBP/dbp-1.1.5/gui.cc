...
InputGui::build() {
  ...
  _selector = gtk_file_chooser_dialog_new("Add Image Files",
  ...
  _clearButton = stdButton("Clear List");
...
TurnGui::build() {
  ...
  _enable = gtk_check_button_new_with_label("Enable");
  ...
  addItem(_turnOptions, "Clockwise", &_turnOptionTags[0], _op._turn);
  addItem(_turnOptions, "Upside-down", &_turnOptionTags[1], _op._turn);
  addItem(_turnOptions, "Anti-clockwise", &_turnOptionTags[2], _op._turn);

  gtk_widget_show_all(_turnOptions);
  gtk_widget_hide(_turnOptions);

  add(_controls, "Turn", _op._turn, _turnOptions);
  if (!_op._enabled) {
...
BlurGui::build() {
  ...
  gtk_box_pack_start(GTK_BOX(vbox), _controls.widget(), FALSE, FALSE, 0);
  add(_controls, "Radius", _op._radius, 1.0, 100.0, 1);
  if (!_op._enabled) {
    gtk_widget_set_sensitive(_controls.widget(), FALSE);
  }
...
RecolourGui::build() {
  ...
  _auto = gtk_check_button_new_with_label("Auto Levels");
  ...
  _manualFrame = gtk_frame_new("Manual");
  ...
  add(_manualControls, "Bright", _op._brightness, -1.0, 1.0, 3);
  add(_manualControls, "Contrast", _op._contrast, -1.0, 1.0, 3);
  add(_manualControls, "Saturation", _op._saturation, 0.0, 2.0, 3);
#if NYI
  add("Gamma", _op._gamma, 0.1, 10.0, 2);
#endif

  _mono = checkButtonFor(_op._mono);
  gtk_button_set_label(GTK_BUTTON(_mono), "Convert to Grey");
...
ResizeGui::build() {
  ...
  addFitOption("Exactly", &_fitOptionTags[0]);
  addFitOption("Padded", &_fitOptionTags[1]);
  addFitOption("Inside", &_fitOptionTags[2]);
  addFitOption("Outside", &_fitOptionTags[3]);
  ...
  _relativeToggle = gtk_radio_button_new_with_label(NULL, "Relative");
  ...
  _fixedToggle = add(_relControls, "Keep Aspect", _op._keepAspect);
  connect(gtkSignal(_fixedToggle, "toggled", &ResizeGui::selectAspect, this));
  _xAdjust = add(_relControls, "X Scale", _op._xScale, 0.01, 2.0, 2);
  connect(gtkSignal(_xAdjust, "value_changed", &ResizeGui::handleXAdjust, this));
  _yAdjust = add(_relControls, "Y Scale", _op._yScale, 0.01, 2.0, 2);
  connect(gtkSignal(_yAdjust, "value_changed", &ResizeGui::handleYAdjust, this));

  _absoluteFrame = gtk_frame_new("Absolute");
  ...
  add(_absControls, "Width", _op._xSize, 1, 6144);
  add(_absControls, "Height", _op._ySize, 1, 4096);
  ...
  setWarning();
  _absControls.add("Fit", _selectFit, _warning);
...
ResizeGui::setWarning() {
  ...
  case ResizeOp::FIT_EXACTLY:
    gtk_label_set_text(GTK_LABEL(_warning), "Note: may change aspect ratio.");
    break;
  case ResizeOp::PAD_TO_FIT:
    gtk_label_set_text(GTK_LABEL(_warning), "Pads with Gimp background colour");
    break;
...
CropGui::build() {
  ...
  add(_controls, "Width", _op._width, 1, 6144);
  add(_controls, "Height", _op._height, 1, 4096);
  add(_controls, "X Pos", _op._x, 0, 6143);
  add(_controls, "Y Pos", _op._y, 0, 4095);
  if (!_op._enabled) {
    gtk_widget_set_sensitive(_controls.widget(), FALSE);
  }
...
SharpenGui::build() {
  ...
  add(_controls, "Radius", _op._radius, 0.1, 120.0, 1);
  add(_controls, "Amount", _op._amount, 0.0, 5.0, 2);
  add(_controls, "Threshhold", _op._threshhold, 0.0, 255.0, 0);
  if (!_op._enabled) {
    gtk_widget_set_sensitive(_controls.widget(), FALSE);
  }
...
RenameGui::build() {
  ...
  _dirSelector = gtk_file_selection_new("Destination Directory");
  gtk_window_set_modal(GTK_WINDOW(_dirSelector), TRUE);

  GtkFileSelection* fs = GTK_FILE_SELECTION(_dirSelector);
  /* Turn a file selector into a directory selector... */
  GtkWidget* files = fs->file_list;
  if (files->parent) {
    files = files->parent;
  }
  gtk_widget_hide(files);
  gtk_widget_hide(fs->selection_entry);
  gtk_file_selection_hide_fileop_buttons(fs);

  connect(gtkSignal(fs->ok_button, "clicked", &RenameGui::okDirSelector, this));
  connect(gtkSignal(fs->cancel_button, "clicked", &RenameGui::cancelDirSelector, this));
  ...
  _controls.add("To Directory:", 0, _dirLabel);

  GtkWidget* hbox = gtk_hbox_new(TRUE, 0);

  GtkWidget* button;

  button = gtk_button_new_with_label("Select Dir");
  connect(gtkSignal(button, "clicked", &RenameGui::raiseDirSelector, this));
  gtk_box_pack_end(GTK_BOX(hbox), button, FALSE, FALSE, 0);

  button = gtk_button_new_with_label("Source Dir");
...
RenameGui::recalcDirName() {
  if (_op._dirPath.empty()) {
    gtk_label_set_text(GTK_LABEL(_dirLabel), "same as source");
  } else {
...
RenameGui::okDirSelector() {
  std::string dir = 
    gtk_file_selection_get_filename(GTK_FILE_SELECTION(_dirSelector));
...
DbpGui::build() {
  ...
  GtkWidget* frame = gtk_frame_new("Processing");
  ...
  _show = Gui::stdToggle("Show Images");
  ...
  //
  add("Input", new InputGui(_data._input));
  add("Turn", new TurnGui(_data._turn));
  add("Blur", new BlurGui(_data._blur));
  add("Colour", new RecolourGui(_data._recolour));
  add("Resize", new ResizeGui(_data._resize));
  add("Crop", new CropGui(_data._crop));
  add("Sharpen", new SharpenGui(_data._sharpen));
  add("Rename", new RenameGui(_data._rename));
  add("Output", new OutputGui(_data._output));
...
DbpGui::test() {
  ...
  if (_data._input._files.empty()) {
    gtk_label_set_text(GTK_LABEL(_messageText), "No file to test with!");
    return;
  }
...
DbpGui::start() {
  ...
  if (n > 0) {
    gtk_label_set_text(GTK_LABEL(_messageText), "Output files exist, please delete them!");
    return;
  }
...
DbpGui::step() {
  if (_data.done()) {
    gtk_label_set_text(GTK_LABEL(_messageText), "-- done --");
    setBusy(false);
    return false;
  }
