...
const FblurStoreParam fblur_init_param = {
  ...
  TRUE,                 /* use depth fill behind */
  TRUE,                 /* use depth fuzzy */
  TRUE,                 /* use shine */
  -1,                   /* depth map id */
  0.0,                  /* focal depth */
  ...
  if (gimp_nparams == 9)
    {
      param->store.model_type =
        CLAMP (gimp_param[3].data.d_int32,
               FBLUR_MODEL_FLAT, FBLUR_MODEL_CONCAVE);
      ...
      param->store.enable_depth_fill_behind = FALSE;
...
focusblur_param_prepare (FblurParam     *param)
{
  ...
  focusblur_diffusion_update (&(param->diffusion),
                              NULL, &(param->store));
...
focusblur_param_store (FblurParam       *param)
{
  ...
  gimp_set_data (PLUG_IN_PROC, &(param->store), sizeof (FblurStoreParam));
}
...
focusblur_param_restore (FblurParam     *param)
{
  ...
  if (gimp_get_data_size (PLUG_IN_PROC) == sizeof (FblurStoreParam) &&
      gimp_get_data (PLUG_IN_PROC, &(param->store)))
