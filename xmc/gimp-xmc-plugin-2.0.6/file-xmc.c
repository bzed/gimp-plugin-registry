 ...
 *
 * In this file, "size" and "dimension" are used in definitely different contexts.
 * "Size" means nominal size of Xcursor which is used to determine which frame
 * depends on which animation sequence and which sequence is really used. (for
 * more detail, please read Xcursor(3).)
 * On the other hand, "Dimension" simply means width and/or height.
 */
 ...
 * environments. This is defined on line 59 of xcursorint.h in libXcursor
 * source code. Make sure this is about dimensions(width and height) not about
 * nominal size despite of it's name. */
#define MAX_BITMAP_CURSOR_SIZE  64

/* The maximum dimension of each frame of X cursor we want to save should be
 * MAX_BITMAP_CURSOR_SIZE but about loading, xhot(& yhot) of each frame varies
 * from 0 to MAX_BITMAP_CURSOR_SIZE-1, so we need to set the maximum dimension
 * of image no less than MAX_BITMAP_CURSOR_SIZE * 2( -1 to be precise) to remain
 * hotspots on the same coordinates.
 * We use four times value (256 for saving, 512 for loading) as a limitation
 * because some cursors generated by CursorXP/FX to X11 Mouse Theme Converter
 * is very large.
 * The biggest cursor I found is "watch" of OuterLimits which size is 213x208.
 * If you found bigger one, please tell me ;-) */
#define MAX_LOAD_DIMENSION      512
#define MAX_SAVE_DIMENSION      256

/* The maximum number of different nominal sizes in one cursor this plug-in
 * can treat. This is based on the number of cursor size which
 * gnome-appearance-properties supports.(12,16,24,32,36,40,48,64)
 * ref. capplets/common/gnome-theme-info.c in source of gnome-control-center */
