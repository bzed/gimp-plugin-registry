...
; 15, 18 mars 2007, 29 janvier, 3 juin 2008
; (c) Jean-Pierre Sutto <jpsmail(at)free.fr>
; Published under GPL version 2


(script-fu-register "diana-holga2b"
	"<Image>/~jps/diana-holga2b"

"This script-fu for The Gimp is a attempt to simulate the Diana/Holga Toys Cameras effect.
Last version can be found at:
http://www.vide.memoire.free.fr/photo/contrefacons/diana-holga2b.scm
...
Modifie' pour tourner sous gimp 2.4 par Ille.
...
(define
	...
	(let*
		(
			...
			(VarMarge (* 0.008 VarDiagonale))
			(VarCorAngle (/ VarAngle 100))
			(VarCropLargeur (* VarCorAngle VarLargeur 2.7))
			(VarCropHauteur (* VarCorAngle VarHauteur 2.7))
			(VarCorBrouillage (* VarFlou 10))
			(VarFakeArray (cons-array 256 'byte))
			(VarZoomTrunc (trunc VarZoom))
			(VarForeOrigine (car (gimp-context-get-foreground)))
			(VarBackOrigine (car (gimp-context-get-background)))
		)
